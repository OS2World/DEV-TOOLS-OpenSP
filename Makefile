...
ACLOCAL = ${SHELL} K:/opensp/opensp-1.5.1/missing --run aclocal-1.6
AUTOCONF = ${SHELL} K:/opensp/opensp-1.5.1/missing --run autoconf
AUTOMAKE = ${SHELL} K:/opensp/opensp-1.5.1/missing --run automake-1.6
AUTOHEADER = ${SHELL} K:/opensp/opensp-1.5.1/missing --run autoheader
...
OBJEXT = o
PATH_SEPARATOR = ;
AMTAR = ${SHELL} K:/opensp/opensp-1.5.1/missing --run tar
AS = @AS@
AWK = awk.exe
...
am__include = include
am__quote = 
install_sh = K:/opensp/opensp-1.5.1/install-sh
...
EXTRA_DIST = instmac.pl msggen.pl sunfix.sh SP.dsw SP.mak \
	...
	OpenSP-1.5.1.spec config.rpath onsgmls.1 \
...
CONFIG_CLEAN_FILES = ${PACKAGE}-${VERSION}.spec msggen.pl intl/Makefile
DIST_SOURCES =
...
DIST_COMMON = README $(pkginclude_HEADERS) ABOUT-NLS AUTHORS COPYING \
	...
	mkinstalldirs msggen.pl.in spec.in
...
${PACKAGE}-${VERSION}.spec: $(top_builddir)/config.status spec.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
msggen.pl: $(top_builddir)/config.status msggen.pl.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
...
distdir: $(DISTFILES)
	...
	$(am__remove_distdir)
	mkdir $(distdir)
	$(mkinstalldirs) $(distdir)/. $(distdir)/include $(distdir)/intl $(distdir)/po
...
installdirs-am:
	$(mkinstalldirs) $(DESTDIR)$(man1dir) $(DESTDIR)$(pkgdocdir) $(DESTDIR)$(pkgincludedir)
...
.PHONY: $(RECURSIVE_TARGETS) GTAGS all all-am check check-am clean \
	...
	install-pkgdocDATA install-pkgincludeHEADERS install-recursive \
